...
export default function NexthmImoveis({ site, imoveis }: TemaProps) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((im) => (
            <div
              key={im.id}
              className="group overflow-hidden rounded-2xl bg-[#f5fafa] shadow-sm transition hover:shadow-xl"
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {im.destaque && (
                  <span
                    className="absolute right-4 top-4 rounded-full px-3 py-1 text-xs font-semibold text-[#122122]"
              ...
              <div className="p-5">
                <h3 className="mb-1 text-lg font-bold text-[#0f393b]">{im.titulo}</h3>
                <p className="mb-3 text-sm text-[#214747]">
                  {im.bairro}, {im.cidade}
                </p>
                <p className="mb-4 text-xl font-bold" style={{ color: primary }}>
                  {formatPrice(im.preco)}
                </p>
                <div className="flex items-center gap-4 border-t border-[#e0eded] pt-3 text-xs text-[#214747]/70">
                  {im.quartos > 0 && (
