...
export default function CrmPage() {
  ...
  const handleSubmit = () => { if (email) { const list = JSON.parse(localStorage.getItem("waitlist_crm") || "[]"); list.push(email); localStorage.setItem("waitlist_crm", JSON.stringify(list)); setSubmitted(true); } };
      ...
      <section className="pt-28 md:pt-36 pb-20 bg-gradient-to-b from-[#FEF2F2] to-white">
        <div className="container mx-auto px-6 max-w-3xl text-center">
          <Reveal className="flex flex-col items-center gap-5">
            ...
            <motion.p variants={fadeUp} className="text-[#6B7280] text-xs">1.203 corretores aguardando</motion.p>
          </Reveal>
        </div>
      </section>
